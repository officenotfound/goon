...
function Cursor({ color }: { color: string }) {
  ...
  return (
    <>
      <div ref={dot} style={{ position: "fixed", top: 0, left: 0, width: 6, height: 6, borderRadius: "50%", background: color, pointerEvents: "none", zIndex: 9999, transition: "background 0.5s" }} />
      <div ref={ring} style={{ position: "fixed", top: 0, left: 0, width: 36, height: 36, borderRadius: "50%", border: `1px solid ${color}`, pointerEvents: "none", zIndex: 9998, opacity: 0.5, transition: "background 0.5s" }} />
    </>
  );
}
...
/* ---------- Theme ---------- */
const D = {
  dark:  { bg: "#080807", text: "#ddd8d0", gold: "#b8966e", dim: "#4a453f", rule: "#171614", sub: "#7a7268", faint: "#232019" },
  light: { bg: "#f4f1ed", text: "#18160f", gold: "#8a6840", dim: "#9a9088", rule: "#dedad4", sub: "#7a7268", faint: "#e8e4de" },
};
...
export default function Home() {
  ...
  return (
    <>
      ...
      <main style={{ background: t.bg, color: t.text, fontFamily: FONT, overflowX: "hidden", transition: "background 0.5s, color 0.5s" }}>
        ...
        <nav style={{
          ...
          background: mode === "dark"
            ? "linear-gradient(to bottom, rgba(8,8,7,0.95) 0%, transparent 100%)"
            : "linear-gradient(to bottom, rgba(244,241,237,0.95) 0%, transparent 100%)",
          ...
          <button onClick={toggle} style={{
            ...
            fontSize: "0.7rem", color: t.dim, fontFamily: FONT, transition: "color 0.4s",
          }}
        ...
        <section style={{ height: "100svh", display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "0 2.8rem 3rem" }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
            <div style={{
              ...
              <span style={{ display: "block", fontSize: "0.35em", color: t.text, marginTop: "-0.05em", marginLeft: "0.35em", letterSpacing: "0.02em", transition: "color 0.5s" }}>PR</span>
            </div>
          </div>
        ...
        {/* TICKER */}
        <div
          style={{ borderTop: `1px solid ${t.rule}`, borderBottom: `1px solid ${t.rule}`, overflow: "hidden", padding: "1rem 0", transition: "border-color 0.5s" }}
          onMouseEnter={() => setTickerHover(true)}
          onMouseLeave={() => setTickerHover(false)}
        >
          <div style={{
            display: "flex", gap: "4rem", whiteSpace: "nowrap",
            animation: `ticker ${tickerHover ? "10s" : "28s"} linear infinite`,
            ...
            {[...TICKER, ...TICKER].map((item, i) => (
              <span key={i} style={{ fontSize: "0.6rem", letterSpacing: "0.22em", textTransform: "uppercase", color: t.dim, flexShrink: 0, transition: "color 0.5s" }}>
                {item} <span style={{ color: t.gold, marginLeft: "1rem" }}>·</span>
              </span>
            ))}
          </div>
        </div>
        ...
        <section style={{ padding: "14rem 2.8rem", maxWidth: "1200px" }}>
          <Reveal>
            <p style={{ fontSize: "clamp(2rem, 5vw, 4.5rem)", fontWeight: 400, lineHeight: 1.2, letterSpacing: "0.005em", color: t.text, transition: "color 0.5s" }}>
              We represent the artists,<br />
              institutions, and collectors<br />
              who shape what culture becomes —<br />
              <em style={{ color: t.gold, transition: "color 0.5s" }}>not after the fact.</em>
            </p>
          </Reveal>
          <Reveal delay={150}>
            <p style={{ fontSize: "0.95rem", fontWeight: 400, lineHeight: 2.1, color: t.dim, marginTop: "5rem", maxWidth: "440px", letterSpacing: "0.02em", transition: "color 0.5s" }}>
        ...
        <section style={{ padding: "10rem 2.8rem" }}>
          ...
          <Reveal delay={60}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 6rem", paddingTop: "3rem", paddingBottom: "3rem", borderTop: `1px solid ${t.rule}`, alignItems: "end", transition: "border-color 0.5s" }}>
              <p style={{ fontSize: "clamp(2.5rem, 6vw, 6rem)", fontWeight: 400, lineHeight: 0.95, letterSpacing: "-0.02em", color: t.text, transition: "color 0.5s" }}>
                Niclas<br />Castello
              </p>
              <div>
                <p style={{ fontSize: "clamp(1.3rem, 2.8vw, 2.4rem)", fontStyle: "italic", fontWeight: 400, color: t.gold, lineHeight: 1.15, marginBottom: "2rem", transition: "color 0.5s" }}>
          ...
          <Reveal delay={60}>
            <div style={{ display: "flex", flexWrap: "wrap", columnGap: "4rem", rowGap: "1.2rem" }}>
              {["Artforum", "The Art Newspaper", "Artnet", "Hyperallergic", "Decrypt", "crypto.news"].map(o => (
                <span key={o} style={{ fontSize: "1.2rem", fontWeight: 400, color: t.dim, letterSpacing: "0.03em", transition: "color 0.5s" }}>
                  {o}
                </span>
              ))}
            </div>
        ...
        <section style={{ padding: "10rem 2.8rem" }}>
          ...
          <Reveal delay={80}>
            <p style={{ fontSize: "clamp(1.6rem, 4vw, 3.5rem)", fontWeight: 400, lineHeight: 1.3, color: t.text, maxWidth: "700px", transition: "color 0.5s" }}>
              Introductions by referral<br />or direct invitation.
            </p>
          </Reveal>
          <Reveal delay={160}>
            <a href="mailto:Dorian.batycka@gmail.com" style={{
              display: "inline-block", marginTop: "3.5rem",
              fontSize: "0.6rem", letterSpacing: "0.22em", textTransform: "uppercase",
              color: t.sub, textDecoration: "none",
              borderBottom: `1px solid ${t.rule}`, paddingBottom: "3px",
              transition: "color 0.4s, border-color 0.4s",
            }}
              onMouseEnter={e => { e.currentTarget.style.color = t.gold; e.currentTarget.style.borderBottomColor = t.gold; }}
              onMouseLeave={e => { e.currentTarget.style.color = t.sub; e.currentTarget.style.borderBottomColor = t.rule; }}
            >
